...
| Ticket | Description | Statut | Date | Commit |
|--------|--------------|--------|------|--------|
| 10.1 | Le brouillard reste figé tel quel à la défaite par timer, au lieu d'être recouvert | Terminé | 2026-08-15 | `f9f7b12` |

Demande directe d'Arnaud (hors lot planifié) : au message « Trop tard ! » (défaite par expiration du timer), le plateau recouvrait jusqu'ici systématiquement toutes les cases (retour au brouillard à 100 %), y compris celles déjà révélées. Arnaud voulait que le plateau reste figé tel quel — cases révélées restent révélées, cases dans le brouillard restent dans le brouillard —, sans que cela affecte l'état initial d'un `Réinitialiser` (qui doit toujours repartir d'un brouillard à 100 %).

Bug localisé dans `tick()` (`server/src/engine/game.ts`) : au passage `timerRemainingMs === 0`, un `setCells(state.revealed, toutes les cases, false)` recouvrait explicitement tout avant de passer en phase `defeat`. Supprimé — la phase passe à `defeat` et l'événement `defeat` est émis, sans plus toucher à `state.revealed`. Vérifié que `reset()`/`initializeBoard()` sont un chemin de code totalement indépendant (`state.revealed = createEmptyRevealed(...)`, jamais partagé avec `tick()`) : aucun risque que ce correctif affecte le comportement de Réinitialiser.

Deux tests mis à jour (`game.test.ts`, `game.integration.test.ts`) pour capturer l'état du brouillard juste avant la défaite et vérifier qu'il est strictement identique juste après (au lieu de vérifier l'ancien recouvrement systématique) ; vérifié aussi qu'aucun événement `revealed_changed` n'est émis par la défaite elle-même. `npm run lint` propre, suite complète verte (244 tests). Vérifié en conditions réelles sur `serveur_dev` (grille 10×10, timer 10s, un joueur qui révèle quelques cases avant l'expiration) : 12/100 cases révélées, état strictement identique juste avant et juste après le passage en `defeat`. Redéploiement (`docker compose up -d --force-recreate`, nécessaire car `--build` seul n'a pas suffi à recréer le conteneur cette fois) : sans persistance, `activeImageId`/`qrUrl` précédemment configurés sur `serveur_dev` ont été perdus au passage — comportement attendu (aucune persistance, cf. contraintes du projet), pas un effet du correctif lui-même.
